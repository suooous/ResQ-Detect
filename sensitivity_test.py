...
def run_experiment(
    ...
    if optimizer_type == 'Adam':
        ...
        optimizer = optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9) # Adding momentum for SGD
...
def main():
    ...
    num_epochs = 30 # Reduced for faster experimentation, set higher for final runs
    ...
    for i, weights in enumerate(class_weights_configs):
        run_experiment(
            ...
            learning_rate=0.001, # Keep other params constant
            optimizer_type='Adam', # Keep other params constant
    ...
    for i, lr in enumerate(learning_rate_configs):
        run_experiment(
            ...
            quantum_depth=5,
            freeze_cnn=False,
            class_weights_val=[1.0, 1.0], # Use a default balanced weight for LR test
            learning_rate=lr,
            optimizer_type='Adam', # Keep optimizer constant
            num_epochs=num_epochs,
            device=device
        )
    ...
    # For optimizer tests, it's often best to find a reasonable learning rate for each optimizer individually
    # However, for a simple sensitivity test, we can use a common LR, or adjust it if an optimizer performs very poorly.
    # Here, I'll use a common LR for initial comparison.
    ...
    for i, opt_type in enumerate(optimizer_configs):
        run_experiment(
            ...
            quantum_depth=5,
            freeze_cnn=False,
            class_weights_val=[1.0, 1.0], # Use a default balanced weight for Optimizer test
